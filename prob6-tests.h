...
{
  Chronometer t1(10, 59, 57);

  ++t1;
  Assert::AreEqual(t1.GetHour(), 10);
  Assert::AreEqual(t1.GetMinute(), 59);
  Assert::AreEqual(t1.GetSecond(), 58);

  ++(++t1);
  Assert::AreEqual(t1.GetHour(), 11);
  Assert::AreEqual(t1.GetMinute(), 0);
  Assert::AreEqual(t1.GetSecond(), 0);
}

TEST_CASE("Chronometer", Chronometer_PostfixIncrementWorks)
{
  Chronometer t1(10, 20, 59);

  const Chronometer t2 = t1++;
  Assert::AreEqual(t1.GetHour(), 10);
  Assert::AreEqual(t1.GetMinute(), 21);
  Assert::AreEqual(t1.GetSecond(), 0);

  Assert::AreEqual(t2.GetHour(), 10);
  Assert::AreEqual(t2.GetMinute(), 20);
  Assert::AreEqual(t2.GetSecond(), 59);
}
...
TEST_CASE("Chronometer", Chronometer_AdditionWorks)
{
  ...
  Assert::AreEqual(t1.GetHour(), 10);
  Assert::AreEqual(t1.GetMinute(), 22);
  Assert::AreEqual(t1.GetSecond(), 45);
}
...
TEST_CASE("Athlete", Athlete_GetBestTimeWorks)
{
  ...
  const Chronometer t1 = at1.GetBestTime();
